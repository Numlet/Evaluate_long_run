...
#    if season!='JJA':continue
    ...
    #    for name in path_dict:
        ...
        plt.subplot(222)        
        jle.Quick_plot(model_data,name,metadata_dataset=ds,levels=levels,lat_bounds=lat_bounds,lon_bounds=lon_bounds,new_fig=0,cb_format='%i')
        model_data.data[model_data.mask]=np.nan
        diff=model_data-lw_dict[season]
    #        diff[diff<-10000]=np.nan
        
        plt.subplot(221)        
        jle.Quick_plot(lw_dict[season],'CMSAF '+ 'Outgoing LW '+season,metadata_dataset=ds,levels=levels,lat_bounds=lat_bounds,lon_bounds=lon_bounds,new_fig=0,cb_format='%i')
        
        plt.subplot(223)
        jle.Quick_plot(diff ,'longwave diff mean '+str(np.nanmean(diff)),metadata_dataset=ds,levels=np.linspace(-50,50,16),cmap=plt.cm.RdBu,lat_bounds=lat_bounds,lon_bounds=lon_bounds,extend=1,new_fig=0,cb_format='%i')
        plt.subplot(224)
    ...
    #        diff[diff<-10000]=np.nan
        ...
        plt.subplot(221)        
        jle.Quick_plot(sw_net_dict[season],'CMSAF net Downward SW '+season,metadata_dataset=ds,levels=levels,lat_bounds=lat_bounds,lon_bounds=lon_bounds,new_fig=0,cb_format='%i')
        
        plt.subplot(223)
        jle.Quick_plot(diff ,'Shortwave diff mean '+str(np.nanmean(diff)),metadata_dataset=ds,levels=np.linspace(-50,50,16),cmap=plt.cm.RdBu_r,lat_bounds=lat_bounds,lon_bounds=lon_bounds,extend=1,new_fig=0,cb_format='%i')
        plt.subplot(224)
        values=diff[~np.isnan(model_data)]
        plt.hist(values,bins=100,normed=1)
        plt.axvline(0,c='b',ls='--')
...
for month in jle.months_number_str:
    ...
    #        diff[diff<-10000]=np.nan
        ...
        plt.subplot(221)        
        jle.Quick_plot(monthly_mean_lw[imonth,],'CMSAF '+ 'Outgoing LW '+month_name,metadata_dataset=ds,levels=levels,lat_bounds=lat_bounds,lon_bounds=lon_bounds,new_fig=0,cb_format='%i')
    ...
    #        diff[diff<-10000]=np.nan
        ...
        plt.subplot(221)        
        jle.Quick_plot(monthly_mean_sw[imonth,],'CMSAF net Downward SW '+month_name,metadata_dataset=ds,levels=levels,lat_bounds=lat_bounds,lon_bounds=lon_bounds,new_fig=0,cb_format='%i')
